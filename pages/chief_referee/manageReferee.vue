<template>
	<cu-custom bgColor="bg-gradual-blue" :isBack="true">
		...
		<block slot="content">裁判信息</block>
	</cu-custom>
	<form>
		<view class="cu-form-group">
			<view class="title">姓名</view>
			<input v-model="newJudge.name" placeholder="输入裁判姓名" name="input" />
		</view>
		<view class="cu-form-group">
		...
		<view class="cu-form-group">
			...
			<!-- <uni-easyinput type="password" v-model="newJudge.password" placeholder="请输入密码"></uni-easyinput> -->
		</view>
...
export default {
	...
	methods: {
		fetchJudges() {
			ChiefRefereeAPI.fetchJudges().then(data => {
		...
		uploadJudge() {
			if (this.editingIndex !== -1) {
				ChiefRefereeAPI.updateJudge(this.newJudge).then((res) => {
					if (res.code === 0) {
						this.fetchJudges();
						this.newJudge = {
							referee_id: '',
				...
				ChiefRefereeAPI.addJudge(this.newJudge).then((res) => {
					if (res.code === 0) {
						this.fetchJudges();
						this.newJudge = {
							referee_id: '',
		...
		deleteJudge(id) {
			ChiefRefereeAPI.deleteJudge(id).then((res) => {
				if (res.code === 0) {
					uni.showToast({
						title: "删除成功",
						icon: 'success',
