	...
	<div v-for="(activity, index) in activities" :key="index" @click="goToActivityDetail(index)">
		<div class="cu-card article" :class="isCard ? 'no-card' : ''">
			<div class="cu-item shadow">
				...
				<view class="btn-warp">
					<button class="btn" @click.stop="goToEditActivity(index)">管理基本信息</button>
					<button class="btn" @click.stop="goToEditReferees(activity.event_id)">管理所属裁判</button>
					<button class="btn" @click.stop="goToTeam(index)">管理参赛队伍</button>
					<button class="btn" @click.stop="goToMatch(index)">管理对战表</button>
				</view>
			</div>
		</div>
	</div>
...
export default {
	...
	methods: {
		...
		fetchActivities() {
			ChiefRefereeAPI.fetchActivitiesByReferee(uni.getStorageSync('token'))
				.then(data => {
		...
		goToEditReferees(eventID) {
			uni.navigateTo({
				url: `/pages/chief_referee/linkActivityAndReferees?eventID=${eventID}`
			});
		},

		goToMatch(index) {
			uni.navigateTo({
				url: `/pages/activities/match?isChiefReferee=true&event=` + JSON.stringify(this.activities[
					index])
			});
		},
		...
		goToTeam(index) {
			uni.navigateTo({
				url: `/pages/chief_referee/activityApplication?event=` + JSON.stringify(this.activities[index])
			});
		},
	},
};
