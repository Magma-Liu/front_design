<template>
    <cu-custom bgColor="bg-gradual-blue" :isBack="true">
        ...
        <block slot="content">非本活动所属裁判列表</block>
    </cu-custom>
    <view>
        <scroll-view scroll-y class="indexes" :scroll-into-view="'indexes-' + listCurID"
            ...
            <block v-for="(group, groupIndex) in list" :key="'group' + groupIndex">
                ...
                <view v-for="(item, index) in group.players" :key="item.referee_id">
                            ...
                            <view class="content">
                                ...
                                <view class="flex solid-bottom padding justify-between">
                                    ...
                                    <button class="cu-btn round sm shadow line-blue"
                                        @click="addRefereeToEvent(eventID, item.referee_id)">添加</button>
                                </view>
                            </view>
                        </view>
                    </view>
...
export default {
    ...
    methods: {
                ...
                .then(data => {
                    ...
                    if (this.list.length === 0) {
                        ...
                        sortedList = data.data
                            .filter(dataItem => !this.list.some(listItem => listItem.referee_id === dataItem.referee_id))
        ...
        // 根据eventID referee_id 指定队员，最后添加成员
        addRefereeToEvent(eventID, referee_id) {
            ChiefRefereeAPI.addRefereeToEvent(eventID, referee_id).then((res) => {
                if (res.code === 0) {
                    this.removeContestantById(referee_id);
                    uni.showToast({
                        title: '添加成功',
                        icon: 'success',
                    });
        ...
        removeContestantById(value) {
            ...
            this.list.forEach(group => {
                ...
                for (let i = 0; i < group.players.length; i++) {
                    if (group.players[i].referee_id === value) {
                        // 找到匹配的referee_id，使用splice移除
